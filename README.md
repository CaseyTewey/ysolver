...
The [public app](https://ysolver-vr0n.onrender.com/) uses Render's free compute plan with no persistent disk. Current games and Undo/Redo are saved in each browser, but server-side completed-game history is temporary and disappears on idle shutdown, restart, or deployment. See [DEPLOYMENT.md](DEPLOYMENT.md) for startup behavior and free-tier limits.
